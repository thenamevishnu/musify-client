...
const Signup = () => {
    ...
    const handleForm = async (event) => {
        ...
        if (!regex.name.test(formData.name)) {
            setClicked(false)
            return setErrorMessage("Name only allowed number, space, 4 - 16 characters and should be start with letter")
        }
        if (!regex.username.test(formData.username)) {
            setClicked(false)
            return setErrorMessage("Username only allowed number, _,  4 - 16 characters and should be start with letter")
        }
        ...
        if (!regex.password.test(formData.password)) {
            setClicked(false)
            return setErrorMessage("Password should be contain at least one upper case, lower case, number, @$!%*?&")
        }
        ...
        <div className='flex justify-center mt-28 px-2 md:px-10'>
            <form className='text-center w-full md:w-[500px] text-white' onSubmit={handleForm} onChange={ handleFormChange }>
                ...
                <div className='flex flex-col sm:flex-row sm:gap-5 w-full'>
                    <div className='flex cursor-pointer relative items-center bg-primary rounded-xl px-3 mt-4 w-full' onClick={() => setFormData({...formData, account_type: "listener"})}>
                    ...
                    <div className='flex cursor-pointer items-center bg-primary relative rounded-xl px-3 mt-4 w-full' onClick={() => setFormData({...formData, account_type: "singer"})}>
